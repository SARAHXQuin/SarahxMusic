...
from sarahxmusic import YouTube, app
from sarahxmusic.core.call import SARAH
from sarahxmusic.misc import SUDOERS, db
from sarahxmusic.utils.database import (
    get_active_chats,
    get_lang,
    get_upvote_count,
    is_active_chat,
...
from sarahxmusic.utils.database import get_assistant
from sarahxmusic.utils.decorators.language import languageCB
from sarahxmusic.utils.formatters import seconds_to_min
from sarahxmusic.utils.inline import close_markup, stream_markup, stream_markup_timer
from sarahxmusic.utils.stream.autoclear import auto_clean
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await SARAH.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                await SARAH.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await SARAH.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
